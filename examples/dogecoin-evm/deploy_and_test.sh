...
# Dogecoin-EVM Cross-Chain HTCL Test Script
# This script demonstrates the complete flow of a cross-chain HTCL transaction

set -e

echo "🚀 Starting Dogecoin-EVM Cross-Chain HTCL Test"
echo "================================================"
...
# Step 1: Alice creates HTCL on Dogecoin
print_status "Step 1: Alice creating HTCL on Dogecoin..."
print_status "Running Alice's Dogecoin script..."
python3 alice_dogecoin_script.py

if [ $? -eq 0 ]; then
    print_success "Alice successfully created HTCL on Dogecoin"
else
...
# Copy dogecoin data to EVM directory
cp ../examples/dogecoin-evm/dogecoin_htcl_data.json .

# Run Bob's EVM script
print_status "Running Bob's EVM script..."
npx hardhat run ../examples/dogecoin-evm/bob_evm_script.js --network localhost
...
npx hardhat run ../examples/dogecoin-evm/alice_evm_withdraw.js --network localhost
...
# Step 4: Bob withdraws on Dogecoin
print_status "Step 4: Bob withdrawing from Dogecoin HTCL..."
cd ../examples/dogecoin-evm

# Copy updated data files
cp ../evm/evm_htcl_data.json .

# Run Bob's Dogecoin withdrawal script
print_status "Running Bob's Dogecoin withdrawal script..."
python3 bob_dogecoin_withdraw.py

if [ $? -eq 0 ]; then
    print_success "Bob successfully withdrew from Dogecoin HTCL"
else
